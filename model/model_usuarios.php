...
function sanitize_rut($r)
{
    return strtolower(str_replace(['.', ' '], '', (string)$r));
}

$id         = isset($_POST['txt_id']) ? trim($_POST['txt_id']) : '';
$rut        = isset($_POST['txt_rut']) ? sanitize_rut($_POST['txt_rut']) : '';
...
if ($nombre === '' || $apellidos === '' || $rut === '' || $email === '' || $tipo === '') {
    http_response_code(400);
    echo json_encode(['codigo' => 2, 'mensaje' => 'Parámetros incompletos']);
    exit;
}
...
try {
    if ($id !== '') {
        ...
        $stmt->fetch();
        $stmt->close();
        if ($dupRut > 0) throw new Exception("Ya existe un usuario con el RUT $rut");
        ...
        $stmt->fetch();
        $stmt->close();
        if ($dupEmail > 0) throw new Exception("Ya existe un usuario con el email $email");

        $sql = "UPDATE usuarios 
            SET rut=?, nombre=?, apellidos=?, contacto=?, email=?, tipo=?, estado=?
            WHERE id=?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("ssssssii", $rut, $nombre, $apellidos, $contacto, $email, $tipo, $estado, $id);
        ...
        $sql = "INSERT INTO usuarios (rut, clave, nombre, apellidos, contacto, email, tipo, estado)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("ssssssis", $rut, $hash, $nombre, $apellidos, $contacto, $email, $tipo, $estado);
        if (!$stmt->execute()) throw new Exception("Error al insertar usuario: " . $stmt->error);
        $stmt->close();
    }
